...
from .utils import add_field_to_objects
...
class VotedQuerySet(QuerySet):
    ...
    def __init__(self, model=None, query=None, using=None, user_id=None, hints=None):
...
class _VotableManager(models.Manager):
            ...
            with transaction.atomic():
                ...
                try:
                    vote = self.through.objects.get(
                        user_id=user_id, content_type=content_type, object_id=self.instance.pk
                    )
                    ...
                    setattr(
                        self.instance, voted_field, getattr(self.instance, voted_field) - 1
                    )
    ...
    def exists(self, user_id, action=UP):
        return self.through.objects.filter(
            user_id=user_id, content_object=self.instance, action=action
        ).exists()
    ...
    def all(self, user_id, action=UP):
        ...
        object_ids = self.through.objects.filter(
            user_id=user_id, content_type=content_type, action=action
    ...
    def user_ids(self, action=UP):
        return (
            self.through.votes_for(self.model, self.instance, action)
            .order_by("-create_at")
            .values_list("user_id", "create_at")
        )
    ...
    def annotate(self, queryset=None, user_id=None, reverse=True, sort=True):
        ...
        return VotedQuerySet(model=queryset.model, query=queryset.query, user_id=user_id)
...
class VotableManager(GenericRelation):
    ...
    def __get__(self, instance, model):
        ...
        manager = self.manager(
            through=self.through, model=model, instance=instance, field_name=self.name
        )
